...
namespace Building_Managment.Views.OwnerCollectionView{
    public partial class OwnerCollectionView : XtraUserControl {
        public OwnerCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Building_Managment.ViewModels.OwnerCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Building_Managment.MyCode.Owner,
