...
namespace Building_Managment.Views.UsersGroupCollectionView{
    public partial class UsersGroupCollectionView : XtraUserControl {
        public UsersGroupCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Building_Managment.ViewModels.UsersGroupCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Building_Managment.MyCode.UsersGroup,
