...
namespace Building_Managment.Views.PaymentMethodCollectionView{
    public partial class PaymentMethodCollectionView : XtraUserControl {
        public PaymentMethodCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Building_Managment.ViewModels.PaymentMethodCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Building_Managment.MyCode.PaymentMethod,
