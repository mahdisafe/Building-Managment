...
namespace Building_Managment.Views.Screen_Priv_TableCollectionView{
    public partial class Screen_Priv_TableCollectionView : XtraUserControl {
        public Screen_Priv_TableCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Building_Managment.ViewModels.Screen_Priv_TableCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Building_Managment.MyCode.Screen_Priv_Table,
