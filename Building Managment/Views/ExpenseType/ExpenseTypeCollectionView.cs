...
namespace Building_Managment.Views.ExpenseTypeCollectionView{
    public partial class ExpenseTypeCollectionView : XtraUserControl {
        public ExpenseTypeCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Building_Managment.ViewModels.ExpenseTypeCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Building_Managment.MyCode.ExpenseType,
